...
const int NUM_PRINTER = 3;
const int NUM_COLOR = 4;
const int REQUIRED_INK = 1e6;
void output(int Case, vector<int> ans) {
    cout << "Case #" << Case + 1 << ": ";
    if (ans.empty()) {
        cout << "IMPOSSIBLE" << endl;
    } else {
        assert(vsum(ans) == REQUIRED_INK);
        for (int s : ans) {
            cout << s << ' ';
        }
        cout << endl;
...
void solve(int Case) {
    mat<int> ink(NUM_COLOR, vi(NUM_PRINTER));
    rep(i, NUM_PRINTER) {
        rep(j, NUM_COLOR) {
            cin >> ink[j][i];
        }
