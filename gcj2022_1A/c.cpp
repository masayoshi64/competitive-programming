...
void solve(int Case) {
    ...
    cin >> E >> W;
    mat<ll> x(E, vl(W));
    mat<vector<ll>> mat_dasu_e(W, mat<ll>(E, vl(XMAX, E)));
    scan(x);
    rep(w, W) {
        rrep(e, 0, E - 1) {
            rep(i, XMAX) {
                if (x[e + 1][w] >= i) {
                    mat_dasu_e[w][e][i] = mat_dasu_e[w][e + 1][i];
                } else {
                    mat_dasu_e[w][e][i] = e + 1;
                }
            }
        }
    }
    ...
    rep(e, E) {
        ...
        ll e_max = e + 1;
        rep(w, W) {
            ll shortage = max(0ll, x[e][w] - (e == 0 ? 0 : x[e - 1][w]));
            rep(i, shortage) {
                ll e_dasu = E;
                rep(j, e + 1, E) {
                    if (x[j][w] < (e == 0 ? 0 : x[e - 1][w]) + i + 1) {
                        e_dasu = j;
                        break;
                    }
                }
                // e_dasu = mat_dasu_e[w][e][(e == 0 ? 0 : x[e - 1][w]) + i + 1];
                chmax(e_max, e_dasu);
                ireru.emplace_back(e_dasu, w);
            }
        }
        while (!weights.empty() && weights.back().first < e_max) {
            Pl x = weights.back();
            count[x.second]--;
            weights.pop_back();
            ans++;
