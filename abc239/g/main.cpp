...
template <typename flow_t> struct Dinic {
    ...
    flow_t max_flow(int s, int t) {
        ...
        while (build_augment_path(s, t)) {
            ...
            flow_t f;
            while ((f = find_min_dist_augment_path(s, t, TINF)) > 0)
                flow += f;
        }
        return flow;
    }
...
int main() {
    ...
    cin >> n >> m;
    Graph g(n);
    g.read(m, -1, false, false);
    vl c(n);
    scan(c);
    Dinic<ll> mf(n * 2);
    for (auto e : g.g[0]) {
        mf.add_edge(0, e.to, inf);
    }
    for (auto e : g.g[n - 1]) {
        mf.add_edge(n + e.to, n - 1, inf);
    }
    rep(i, 1, n - 1) {
        for (auto e : g.g[i]) {
            if (e.to == 0 || e.to == n - 1)
                continue;
            mf.add_edge(i, e.to, inf);
            mf.add_edge(n + i, n + e.to, inf);
        }
        mf.add_edge(i, n + i, c[i]);
    }
    ll f = mf.max_flow(0, n - 1);
    print(f);
}